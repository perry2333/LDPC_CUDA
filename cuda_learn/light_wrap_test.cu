...
__global__ void arithmetic(real *d_x, const real x0, const int N) {
    ...
    if (idx < N) {
        real x = d_x[idx];
        while (sqrtf(x * 1000.0f) < x0) {
            x += 1.0f;
        }
        d_x[idx] = x;
    }
}
__global__ void light_compute_kernel(float* data, const float target, const int N) {
...
float run_kernel(int threads_per_block, int num_blocks, real *d_x, real x0, int N) {
    ...
    light_compute_kernel<<<num_blocks, threads_per_block>>>(d_x, x0, N);
...
int main() {
    ...
    int thread_counts[num_configs] = {63,64,65, 500, 513, 1000, 1024};
